...
file_name = 'ssh_exec_cmd_output.txt'
log_file = '%s\\%s'%(tool_folder, file_name)
fo = open(log_file, 'w+')
...
# cmd_file_part = "/ws/%s-bgl/pyATS/pyats/users/%s/runinfo/%s/TaskLog.__task*"%(executor,executor,jobfile_name)
cmd_file_part = "/ws/%s-bgl/pyATS/users/%s/runinfo/%s/TaskLog.__task*"%(executor,executor,jobfile_name)
cmd_cut_part  =  "| cut -d\":\" -f8"
#cmd_cut_part  =  "| cut -d\":\" -f9"
...
cmd2 = "grep \": %s\" /ws/%s-bgl/pyATS/users/%s/runinfo/%s/TaskLog.__task1 | cut -d\":\" -f8"%(grep_str_running, executor, executor, jobfile_name)
#cmd2 = "grep \": %s\" /ws/%s-bgl/pyATS/pyats/users/%s/runinfo/%s/TaskLog.__task* | cut -d\":\" -f9"%(grep_str_running, executor, executor, jobfile_name)
