...
<?php
    ...
    if ($_POST['parse_logs_btn']) {
        ...
        $getchkboxver = $_POST['home_chkbxVer'];
        // print("getchkboxver=\"$getchkboxver\"");  // When checked this prints: getchkboxver="on" else getchkboxver=""
        $status_updated = false;
        $print_status = false;
        $emsg = '';
        $verbose = 0;
        #$tool_folder = 'C:\\xampp\\htdocs\\hostnameer\\dashboard\\run_status';
        $tool_folder = 'C:\\xampp\\htdocs\\dashboard\\run_status';
        if (trim($getcecuser)) {
            if (trim($getcecpassword)) {
                if (trim($getjobfilename)) {
					...
					#$pyscript = 'c:\\xampp\\htdocs\\hostnameer\\dashboard\\run_status\\ssh_exec_cmd.py';
					$pyscript = "$tool_folder\\ssh_exec_cmd.py";
                    #$python = 'C:\\Python36\\python.exe'; # ==> This should match with the python path of the server-system 
                    #$python = 'C:\\Python37\\python.exe';
					#$python = 'C:\\Program Files\\Python37\\python.exe';
					$python = 'C:\\Python\\python.exe';
					#$python = 'python.exe';
                    $cmd = "\"$python\" $pyscript $getcecuser $getcecpassword \"$getjobfilename\" \"$getexecutor\" \"$verbose\"";
					#print($cmd);
    ...
    if ($status_updated) {
        ...
        <form action='./run_status.php' method='post'>
            ...
            <tr>
                <td class='w3-label w3-text-teal'>Get Detailed Table<br>
                </td>
                <td><input class='w3-check' type='checkbox' id='chkVerbose' name='home_chkbxVer' <?php if ($getchkboxver=="on") echo "checked='checked'";?>>
                
                <font size="1"><i>Check this if you need verbose output</i></font>
                </td>
            </tr>
